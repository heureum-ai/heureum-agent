...
def _is_known_poll_tool(
    tool_name: str,
    dynamic_poll_tools: frozenset = frozenset(),
) -> bool:
    """Check if a tool is a known polling/wait tool.

    Checks client-provided dynamic poll set first, then static fallbacks.
    """
    return tool_name in dynamic_poll_tools or tool_name in _KNOWN_POLL_TOOLS
...
def detect_tool_call_loop(
    ...
    Args:
        dynamic_poll_tools: Client-provided poll tool names from ``tool_meta.poll``.
    """
    cfg = config or _default_config
    if not cfg.enabled:
    ...
    if _is_known_poll_tool(last_tool, dynamic_poll_tools) and no_progress >= cfg.critical_threshold:
        return LoopDetectionResult(
