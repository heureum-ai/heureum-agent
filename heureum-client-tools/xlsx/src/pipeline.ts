...
export type XlsxTaskContext = TaskContext;

const DEFAULT_CACHE_DIR_NAME = XLSX_DEFAULTS.runtime.cacheDirName;

export const XLSX_STEP_SOURCE = "steps/01_source";
export const XLSX_STEP_PARSED = "steps/02_parsed";
export const XLSX_STEP_INTERMEDIATE = "steps/03_intermediate";
export const XLSX_STEP_OUTPUT = "steps/04_output";

export function resolveXlsxWorkDir(workDir?: string): string {
...
export class XlsxPipeline {
  private readonly _taskDir: string;

  constructor(ctx: XlsxTaskContext) {
    const base = resolveXlsxWorkDir(ctx.workDir);
  ...
  ensureStepDirs(): void {
    fs.mkdirSync(this.stepPath(XLSX_STEP_SOURCE), { recursive: true });
    fs.mkdirSync(this.stepPath(XLSX_STEP_PARSED), { recursive: true });
    fs.mkdirSync(this.stepPath(XLSX_STEP_INTERMEDIATE), { recursive: true });
    fs.mkdirSync(this.stepPath(XLSX_STEP_OUTPUT), { recursive: true });
  }
...
export function xlsxPipelineFromTaskDir(taskDir: string): XlsxPipeline {
  ...
  return new XlsxPipeline({ sessionId, taskType, taskId, workDir });
}
