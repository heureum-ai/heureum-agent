...
export type MdTaskContext = TaskContext;

const DEFAULT_CACHE_DIR_NAME = MD_DEFAULTS.runtime.cacheDirName;

export const MD_STEP_SOURCE = "steps/01_source";
export const MD_STEP_PARSED = "steps/02_parsed";
export const MD_STEP_INTERMEDIATE = "steps/03_intermediate";
export const MD_STEP_OUTPUT = "steps/04_output";

export function resolveMdWorkDir(workDir?: string): string {
...
export class MdPipeline {
  private readonly _taskDir: string;

  constructor(ctx: MdTaskContext) {
    const base = resolveMdWorkDir(ctx.workDir);
  ...
  ensureStepDirs(): void {
    fs.mkdirSync(this.stepPath(MD_STEP_SOURCE), { recursive: true });
    fs.mkdirSync(this.stepPath(MD_STEP_PARSED), { recursive: true });
    fs.mkdirSync(this.stepPath(MD_STEP_INTERMEDIATE), { recursive: true });
    fs.mkdirSync(this.stepPath(MD_STEP_OUTPUT), { recursive: true });
  }
...
export function mdPipelineFromTaskDir(taskDir: string): MdPipeline {
  ...
  return new MdPipeline({ sessionId, taskType, taskId, workDir });
}
