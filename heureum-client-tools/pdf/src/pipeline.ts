...
export type PdfTaskContext = TaskContext;

const DEFAULT_CACHE_DIR_NAME = PDF_DEFAULTS.runtime.cacheDirName;

export const PDF_STEP_SOURCE = "steps/01_source";
export const PDF_STEP_PARSED = "steps/02_parsed";
export const PDF_STEP_INTERMEDIATE = "steps/03_intermediate";
export const PDF_STEP_OUTPUT = "steps/04_output";

export function resolvePdfWorkDir(workDir?: string): string {
...
export class PdfPipeline {
  ...
  constructor(ctx: PdfTaskContext) {
    const base = resolvePdfWorkDir(ctx.workDir);
  ...
  ensureStepDirs(): void {
    fs.mkdirSync(this.stepPath(PDF_STEP_SOURCE), { recursive: true });
    fs.mkdirSync(this.stepPath(PDF_STEP_PARSED), { recursive: true });
    fs.mkdirSync(this.stepPath(PDF_STEP_INTERMEDIATE), { recursive: true });
    fs.mkdirSync(this.stepPath(PDF_STEP_OUTPUT), { recursive: true });
  }
...
export function pdfPipelineFromTaskDir(taskDir: string): PdfPipeline {
  ...
  return new PdfPipeline({ sessionId, taskType, taskId, workDir });
}
