...
describe('handleWebTool', () => {
  ...
  it('saves JSON and .md files to working directory', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'web-test-'))
    try {
      await handleWebTool(FETCH_TOOL, {
        url: 'https://example.com',
        working_directory: tmpDir,
        session_id: 'sid',
      })
      ...
      expect(jsonFiles[0]).toMatch(/^fetch_example_com_\d+\.json$/)
      expect(mdFiles.length).toBe(1)
      expect(mdFiles[0]).toMatch(/^fetch_example_com_\d+\.md$/)

      const saved = JSON.parse(fs.readFileSync(path.join(sessionDir, jsonFiles[0]), 'utf-8'))
      expect(saved.url).toBe('https://example.com')
      expect(saved.status).toBe(200)

      // .md should contain clean markdown without JSON/security wrapper
  ...
  it('returns raw result when no working directory', async () => {
    const result = await handleWebTool(FETCH_TOOL, { url: 'https://example.com' })
    expect(result.success).toBe(true)
  ...
  it('handles invalid URL gracefully', async () => {
    const result = await handleWebTool(FETCH_TOOL, { url: 'not-a-url' })
