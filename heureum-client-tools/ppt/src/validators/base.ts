...
export class BaseSchemaValidator {
  ...
  private async _validateSingleFileXsd(
    ...
    if (!(await pathExists(fullSchemaPath))) return [null, null];

    try {
      const xsdStr = (await fs.promises.readFile(fullSchemaPath, "utf-8"));
      const xsdDoc = libxmljs.parseXml(xsdStr, { baseUrl: fullSchemaPath });

      let xmlStr = (await fs.promises.readFile(xmlFile, "utf-8"));
  ...
  private async _getOriginalFileErrors(xmlFile: string): Promise<Set<string>> {
    ...
    const tmpDir = (await fs.promises.mkdtemp(path.join(os.tmpdir(), "xsd-orig-")));
    try {
      ...
      for (const [zipPath, zipEntry] of Object.entries(zip.files)) {
        if (zipEntry.dir) {
          ;(await fs.promises.mkdir(path.join(tmpDir, zipPath), { recursive: true }));
        } else {
      ...
      if (!(await pathExists(originalXmlFile))) return new Set();
