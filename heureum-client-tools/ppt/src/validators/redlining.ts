...
export class RedliningValidator {
  ...
  async validate(): Promise<ValidationResult> {
    ...
    if (!(await pathExists(modifiedFile))) {
    ...
    try {
      const modContent = (await fs.promises.readFile(modifiedFile, "utf-8"));
    ...
    try {
      const buffer = (await fs.promises.readFile(this.originalDocx));
